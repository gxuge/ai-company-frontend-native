...
export function DiscoveryListLayout({
  ...
  return (
    <div className="relative mx-auto flex min-h-dvh w-full max-w-[520px] flex-col bg-black text-white">
      <img
        alt=""
        src={imgPageBg}
        className="pointer-events-none absolute inset-x-0 top-0 h-[660px] w-full object-cover opacity-90"
      />

      <div className="relative flex-1 overflow-y-auto pb-[92px]">{children}</div>

      <nav className="sticky bottom-0 z-10 h-[58px] shrink-0">
        <img
          alt=""
          src={imgNavBg}
                ...
                <span className="relative flex h-[21px] items-center justify-center">
                  ...
                  {item.dot && (
                    <img
                      alt=""
                      src={item.dot}
                      className="absolute -top-[1px] -right-[3px] size-[8px]"
                    />
                  )}
                </span>
                <span
  ...
  return (
    <header className="flex items-end gap-[18px] px-[16px] pt-[14px]">
      <div className="relative">
        <span className="font-bold text-[20px] leading-none text-[#a0cd27]">发现</span>
        <img
          alt=""
          src={imgSparkle}
          ...
          <span
            className={
              activeTab === tab
                ...
                : 'font-bold text-[13px] leading-none text-[#8a8a8a]'
            }
          >
            {tab}
          </span>
        ...
        return (
          <button
            ...
            className={`flex shrink-0 items-center gap-[5px] rounded-full border px-[12px] py-[6px] transition-[border-color,background-color,color,transform] duration-150 ease-out active:scale-90 ${
              active
                ? 'border-[#97ed08] bg-[#030204] ring-1 ring-inset ring-[#97ed08]'
...
export function DiscoverySectionHeader({
  ...
  return (
    <div className="flex items-center">
      ...
      <h3 className="font-bold text-[12px] leading-none text-[#a7a7a7]">{title}</h3>
