...
The app uses modular locale folders under `src/locales`, with one folder per language and one JSON file per feature. Add the language resources there, then register the language code in `src/lib/i18n/resources.ts`.
